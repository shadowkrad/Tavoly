...
export default async function HomePage() {
  const [tenantConfig, areas, tables, reservations] = await Promise.all([
    getTenantConfig(),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Monitoraggio occupazione tavoli, turni e flusso ospiti in tempo reale
          </p>
        </div>
      ...
      {/* Main Grid: Left side Tables, Right side Reservations */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        <div className="lg:col-span-7 xl:col-span-8">
          <TableGrid tables={tables} areas={areas} />
        </div>
